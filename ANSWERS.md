...
## Классификация регистровых архитектур по количеству операндов, которые могут быть загружены в АЛУ напрямую
...
Архитектуры процессоров классифицируются по способу организации набора инструкций (ISA) и реализации их выполнения. Основные категории включают `CISC`, `RISC`, `VLIW` и `EPIC`. Рассмотрим их подробнее:
...
## **Кодирование инструкций в MIPS**

MIPS является архитектурой с фиксированной длиной инструкций. Каждая инструкция занимает ровно 32 бита и имеет строгую структуру. Существует три основных формата инструкций:
...
- **R-формат (регистр-регистр)**:
   ...
   **Пример**: сложение (add $t1, $t2, $t3)
   ...
   - Итоговая инструкция: `000000 01010 01011 01001 00000 100000`.
...
- **I-формат (регистр-память)**:
   ...
   **Пример**: загрузка (lw $t1, 4($t2))
   - **Разбор**:
     - opcode: `100011` (код для lw),
     - rs: `01010` ($t2),
     - rt: `01001` ($t1),
     - immediate: `0000 0000 0000 0100` (4 в 16-битной форме).
   - Итоговая инструкция: `100011 01010 01001 0000 0000 0000 0100`.
...
### **Пример инструкции x86**

**Пример**: сложение (add eax, ebx)
- **Разбор**:
  - Префиксы: отсутствуют.
  - Opcode: `03` (add с использованием ModR/M).
  - ModR/M: `C3` (mod = `11` (оба операнда - регистры), reg = `000` (eax), r/m = `011` (ebx)).
- Итоговая инструкция: `03 C3`.

## Сравнение MIPS и x86

| Характеристика         | MIPS                     | x86                      |
|-|--|--|
| Длина инструкции        | Фиксированная (32 бита) | Переменная (1-15 байт)   |
| Простота декодирования  | Простая                 | Сложная                 |
| Адресация памяти        | Непосредственно         | Гибкая (через SIB/ModR/M)|
| Ориентированность       | Производительность      | Универсальность          |
...
| Тип инструкции         | Пример                     | Назначение                         |
|-|-|-|
| Арифметические         | `ADD`, `SUB`              | Выполнение математических операций |
| Логические             | `AND`, `OR`, `XOR`        | Побитовая обработка данных         |
| Передача данных        | `LOAD`, `STORE`, `MOV`    | Перемещение данных                 |
| Управление потоком     | `JMP`, `CALL`, `RET`      | Изменение порядка выполнения       |
| Сдвиги и вращения      | `SHL`, `ROR`              | Работа с битами в регистрах        |
| Сравнение              | `CMP`, `SLT`             | Сравнение значений                 |
| Системные              | `SYSCALL`, `HLT`          | Управление состоянием процессора   |
| Векторные              | `ADDPS`, `MULPD`          | Обработка массивов и потоков       |
| Работа с памятью       | `INVLPG`, `LDS`           | Доступ и управление памятью        |
| Числа с плавающей точкой | `FADD`, `FDIV`           | Математика с плавающей точкой      |
...
## **Регистровый файл с изменяемой разрядностью**
...
## **Статический оконный регистровый файл**
...
## **Динамический оконный (регистровый стек) файл**
...
## Сравнение типов регистровых файлов

| Тип регистрового файла              | Преимущества                                         | Недостатки                                          |
|-|--|--|
| **Классический регистровый файл**   | Простота, универсальность.                         | Нет специализации для различных типов данных.       |
| **Стековый регистровый файл**       | Эффективность при вызове подпрограмм.              | Ограниченная гибкость для сложных вычислений.       |
| **Регистровый файл с изменяемой разрядностью** | Оптимизация использования регистров.                | Усложнение архитектуры и обработки команд.          |
| **Статический оконный регистровый файл** | Быстрый доступ к данным для подпрограмм.           | Сложность в управлении окнами.                      |
| **Динамический оконный регистровый файл** | Гибкость и оптимизация использования регистров.   | Сложность в управлении и высокие вычислительные затраты. |

Выбор типа регистрового файла зависит от специфики задач, которые должен решать процессор, и от особенностей его архитектуры.
...
### **Регистр (CPU Registers)**
Это наименьший и самый быстрый тип памяти, которая непосредственно доступна процессору. Регистр используется для хранения промежуточных данных, операндов, адресов и флагов. Количество регистров ограничено и обычно составляет несколько десятков.

### **Кэш-память (Cache Memory)**
Кэш-память используется для хранения наиболее часто используемых данных и инструкций, с целью ускорения доступа к ним. Кэш-память обычно разделяется на несколько уровней:
...
Основная память, используемая для хранения данных и программ во время выполнения. В оперативной памяти данные могут быть быстро изменены. Оперативная память разделяется на два типа:
...
### **Виртуальная память**
Виртуальная память позволяет операционной системе использовать жесткий диск или другие устройства хранения данных как дополнение к физической памяти. Это создаёт иллюзию, что система имеет больше оперативной памяти, чем есть на самом деле. 

## **Устройство управления памятью (MMU)**

**MMU (Memory Management Unit)** — это аппаратный компонент, который управляет доступом к памяти и предоставляет функции виртуализации памяти. Основные задачи MMU включают:
- **Трансляция виртуальных адресов в физические**.
- **Защита памяти**: предотвращение доступа к защищённым областям памяти.
- **Поддержка виртуальной памяти**: использование части жесткого диска в качестве расширенной памяти.

### **Основные функции MMU**

#### **Трансляция адресов (Address Translation)**
Процессор работает с виртуальными адресами, но для доступа к памяти эти адреса должны быть преобразованы в физические. MMU выполняет трансляцию виртуальных адресов в физические с помощью таблиц страниц.

#### **Управление кэшированием (Cache Management)**
MMU также может управлять кэшированием, например, кэшировать данные из основной памяти для повышения производительности. Это позволяет ускорить доступ к часто используемым данным.

#### **Защита памяти (Memory Protection)**
MMU обеспечивает защиту памяти, чтобы одна программа не могла случайно или намеренно изменить данные другой программы. Для этого она использует различные механизмы, такие как разрешения на доступ (чтение, запись, исполнение).

#### **Виртуализация памяти (Virtual Memory Management)**
MMU поддерживает виртуальную память, что позволяет операционной системе создавать большие адресные пространства, чем физическая память, посредством механизма страницы и свопинга.
...
### **Виртуальный адрес**
Когда процессор пытается обратиться к определенному адресу, этот адрес является виртуальным. Виртуальные адреса должны быть переведены в физические для работы с памятью.

### **Трансляция через таблицы страниц**
Таблицы страниц, хранящиеся в памяти, содержат информацию о маппинге виртуальных страниц в физические. MMU использует эти таблицы для перевода виртуальных адресов в физические.

### **Преобразование адреса**
MMU использует разделение виртуального адреса на две части:
...
MMU преобразует виртуальный номер страницы в физический адрес через таблицу страниц.

### **Кэширование и защита**
MMU может также кешировать переведённые адреса в специальных кэшах, таких как **TLB (Translation Lookaside Buffer)**, для ускорения повторных трансляций.
...
### **Одноступенчатая таблица**
Используется в более простых системах, где один уровень таблицы отвечает за преобразование виртуальных адресов в физические.

### **Многоступенчатая таблица**
В более сложных системах используется многоступенчатая таблица, где виртуальный адрес делится на несколько частей. Каждая часть указывает на очередной уровень таблицы, пока не будет найден конечный физический адрес.

## **Преимущества и недостатки MMU**

| Преимущества                                           | Недостатки                                           |
|--||
| Позволяет использовать виртуальную память и увеличивать объем доступной памяти. | Дополнительная нагрузка на процессор из-за трансляции адресов. |
| Обеспечивает защиту памяти и предотвращение несанкционированного доступа. | Может привести к снижению производительности из-за частых обращений к таблицам страниц. |
| Упрощает многозадачность и управление процессами в операционных системах. | Сложность в реализации и увеличение потребности в памяти для хранения таблиц. |

**MMU** играет ключевую роль в организации памяти и управлении её ресурсами, обеспечивая эффективное использование памяти, защиту данных и поддержку виртуализации.
...
## **Задачи виртуальной памяти и TLB**

### **Задачи распределения реальной памяти между процессами**

Операционная система должна эффективно распределять физическую память между множеством процессов. Виртуальная память помогает решать эту задачу, позволяя каждому процессу иметь свой собственный адресный простор, который не зависит от реального расположения данных в памяти.

### **Задача защиты данных**

Виртуальная память обеспечивает защиту данных, как между процессами, так и внутри одного процесса. Каждый процесс имеет свой виртуальный адресный простор, и операционная система с помощью MMU и TLB следит за тем, чтобы один процесс не мог получить доступ к памяти другого процесса.

### **Атрибуты процессора с поддержкой многозадачной ОС**

Современные процессоры, поддерживающие многозадачные операционные системы, включают в свой состав MMU и TLB, которые позволяют эффективно управлять памятью, поддерживать виртуальную память и изоляцию процессов.

## **Очищение TLB при переключении процесса**
...
- Определение: Если к процессору обращаются данные из памяти, то с большой вероятностью в ближайшее время потребуются соседние данные.

### **Временная локальность**:
- Определение: Если к процессору обращаются данные из памяти, то с большой вероятностью эти же данные будут запрошены снова в ближайшем будущем.

# 13. Кэш: логическая организация, ассоциативность. Поиск данных в КЭШе


## Логическая организация кэша, ассоциативность и поиск данных

### 1. **Логическая организация кэша**

Логическая организация кэша определяется способом хранения и поиска данных. Основные модели:

#### a) **Прямое отображение (Direct Mapping)**  
Каждый блок основной памяти (RAM) может быть размещен только в одной фиксированной строке кэша. Это достигается с помощью вычисления индекса строки:
...
#### b) **Полностью ассоциативный кэш (Fully Associative Cache)**  
Любой блок памяти может быть размещен в любой строке кэша. Поиск осуществляется через полное сравнение тегов (адресов).
...
#### c) **Кэш множественно-ассоциативным отображением (Set-Associative Cache)**  
Кэш делится на наборы, и каждый блок памяти может быть размещен в любом месте внутри заданного набора. Индекс строки рассчитывается как:

Index = (Адрес блока памяти) mod (Количество наборов)

Ассоциативность определяет количество строк в наборе (например, 2- или 4-ассоциативный кэш).  
...
### 3. **Поиск данных в кэше**

Поиск данных в кэше включает следующие этапы:

#### a) **Разделение адреса памяти**
Адрес делится на три части:
1. **Tag (тег):** для идентификации блока.
2. **Index (индекс):** для определения строки (в прямом или наборно-ассоциативном кэше).
3. **Block Offset (смещение):** для адресации внутри блока.

#### b) **Процедура поиска**
1. Используя индекс, определяется потенциальная строка или набор строк.
2. Тег запрашиваемого адреса сравнивается с тегами строк в наборе.
3. Если тег совпадает, это **попадание (hit)**, и данные извлекаются.
4. Если тег не найден, это **промах (miss)**, и данные загружаются из основной памяти.
...
## **Типы протоколов когерентности кэша**

### a) **Протоколы на основе записи (Write Protocols)**

- **MESI (Modified, Exclusive, Shared, Invalid)**  
  Это один из наиболее известных протоколов когерентности. Каждый блок в кэше может находиться в одном из четырех состояний:
...
- **MOESI (Modified, Owned, Exclusive, Shared, Invalid)**  
  Этот протокол расширяет MESI и добавляет состояние **Owned**, которое указывает, что кэш содержит данные, которые должны быть записаны в основную память при следующей записи.

### b) **Протоколы на основе состояния (State-Based Protocols)**

Протоколы когерентности на основе состояний работают, отслеживая, в каком состоянии находится кэш каждого процессора, и координируют обмен данными между кэшами.

- **MSI (Modified, Shared, Invalid)**  
  Это упрощенная версия MESI, которая ограничивает возможные состояния до трех: **Modified**, **Shared**, и **Invalid**. Этот протокол проще в реализации, но менее эффективен в случаях, когда требуется высокая производительность.
...
Векторные архитектуры — это тип архитектуры процессоров, которые используют векторные инструкции для параллельной обработки данных. Они ориентированы на выполнение операций над массивами данных, что позволяет значительно увеличить производительность в задачах, требующих обработки больших объемов данных, таких как научные вычисления, обработка сигналов и графика.

## 1. **Что такое векторная архитектура?**
...
## 2. **Примеры векторных архитектур**
...
## 3. **Преимущества векторных архитектур**

- **Параллельная обработка:** Возможность обработки нескольких данных одновременно.
- **Высокая производительность:** Особенно в задачах, требующих многократных вычислений на больших объемах данных.
- **Эффективность:** Меньше тактов для выполнения операций с массивами данных.
...
- **Ускорение обработки данных**: SIMD позволяет ускорить выполнение операций над большими массивами данных за счет параллельной обработки.
- **Экономия энергии**: Выполнение нескольких операций за один такт позволяет сократить потребление энергии по сравнению с выполнением тех же операций по очереди.
- **Увеличение производительности**: Особенно эффективно для задач с высокой вычислительной нагрузкой, таких как обработка изображений, аудио, видео и научные вычисления.
...
### a) **Конфликты данных (Data Hazards)**
Конфликты данных возникают, когда одна инструкция зависит от данных, которые еще не были вычислены предыдущей инструкцией.
...
### b) **Конфликты управления (Control Hazards)**
Конфликты управления происходят, когда процессор не знает, какая инструкция будет следующей из-за переходов (например, условных операторов или циклов).
...
### c) **Конфликты структур (Structural Hazards)**
Конфликты структур возникают, когда аппаратные ресурсы процессора не могут одновременно обслуживать несколько инструкций. Это может произойти, когда для двух инструкций требуется один и тот же ресурс.
...
### a) **Перенаправление (Forwarding)**
При конфликтах данных процессор может перенаправлять результат выполнения одной инструкции на вход следующей, чтобы избежать задержек.

### b) **Задержка (Stalling)**
Когда невозможно разрешить конфликт, процессор может приостановить выполнение некоторых инструкций (поставить их в ожидание) до разрешения конфликта.

### c) **Предсказание переходов (Branch Prediction)**
Для разрешения конфликтов управления используются алгоритмы предсказания переходов, чтобы заранее определить, куда будет направляться выполнение программы.

### d) **Увеличение числа ресурсов**
Для разрешения структурных конфликтов процессоры могут использовать дополнительные ресурсы, такие как отдельные блоки для чтения и записи памяти.
...
### a) **Предсказание переходов**
- **Основной метод для снижения потерь**. Пример: процессор заранее предсказывает направление перехода и начинает выполнение команд, как будто предсказание верно.
...
- Если конвейер перезагружается после неверного предсказания перехода, **производительность процессора** значительно падает.
- Конвейерная обработка эффективна при большом числе последовательно выполненных команд, а перезагрузка приводит к **потерям производительности** из-за очистки буферов и повторного выполнения инструкций.
...
# Механизмы ускорения выборки инструкций
...
## 1. **Внеочередное исполнение инструкций (Out-of-Order Execution)**
...
## 3. **Технологии микро- и макро-fusion (Micro-fusion, Macro-fusion)**
